...
public class LCManager {
    ...
    public boolean quorumHighest(int timestamp, byte[] value, HashSet<CollectData> collects) {
        ...
        if (appears)
            logger.debug("timestamp/value pair (" + timestamp + " , " + Arrays.toString(value) + ") appears in at least one replica context");
...
//                bftsmart.tom.util.Logger.println("\t\t[CERTIFIED VALUE] " + Arrays.toString(value) + "  == " + Arrays.toString(pv.getValue()));
    ...
    public boolean hasValidProof(CertifiedDecision cDec) {
        ...
        logger.info("Verifying proof for decision cid: " + cDec.getCID() + " pid: " + cDec.getPID());
            ...
            if (consMsg.getProof() instanceof byte[]) { // certificate is made of signatures
                ...
                if (Arrays.equals(consMsg.getValue(), hashedValue)) {
                    if (TOMUtil.verifySignature(pubKey, data, signature)) {
                        if (!alreadyCounted.contains(consMsg.getSender())) {
                            alreadyCounted.add(consMsg.getSender());
                            logger.info("Added sender " + consMsg.getSender() + " for msg epoch " + cm.getEpoch() + " number " + cm.getNumber());
                            countValid++;
                        } else {
                            logger.info("ORA FODASSSS!!! Already contained " + consMsg.getSender() + " for msg epoch " + cm.getEpoch() + " number " + cm.getNumber());
                        }
                    } else {
                        logger.info("ARDEU AQUI!!!");
                    }
                } else {
