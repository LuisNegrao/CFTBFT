...
public class ServersCommunicationLayer extends Thread {
    ...
    public ServersCommunicationLayer(ServerViewController controller,
                                     ...
                                     ServiceReplica replica, LinkedBlockingQueue<SystemMessage> switchingQueue) throws Exception {

        this.controller = controller;
        this.inQueue = inQueue;
        this.me = controller.getStaticConf().getProcessId();
        this.replica = replica;
        this.switchingQueue = switchingQueue;
    ...
    private ServerConnection getConnection(int remoteId) {
        ...
        if (ret == null) {
            ret = new ServerConnection(controller, null,
                    remoteId, this.inQueue, this.replica, switchingQueue);
        ...
        for (int target : targetsShuffled) {
            try {
                if (target == me) {
                    ...
                    if (sm instanceof TriggerMessage || sm instanceof SwitchMessage || sm instanceof ConfirmationMessage) {

                        try {
                            controller.queueLock.lock();
                            switchingQueue.offer(sm);
                        } finally {
        ...
        if ((this.controller.getStaticConf().getTTPId() == remoteId) || this.controller.isCurrentViewMember(remoteId)) {
            ...
            if (this.connections.get(remoteId) == null) { //This must never happen!!!
                ...
                this.connections.put(remoteId,
                        new ServerConnection(controller, newSocket, remoteId, inQueue, replica, switchingQueue));
            } else {
                //reconnection	
